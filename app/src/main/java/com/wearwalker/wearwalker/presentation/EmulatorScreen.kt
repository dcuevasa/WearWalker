...
import com.wearwalker.wearwalker.R
...
private fun EmulatorDebugScreen(
    ...
    val lines = buildList {
        ...
        add("Captured companions (session): ${uiState.caughtCompanionCount}")
        ...
        if (snapshot != null) {
            add("${stringResource(R.string.owner_label)}: ${snapshot.ownerName}")
            ...
            add(
                "${stringResource(R.string.flags_label)}: " +
                    "registered=${snapshot.isRegistered}, hasCompanion=${snapshot.hasCompanion}",
            )
            add(
