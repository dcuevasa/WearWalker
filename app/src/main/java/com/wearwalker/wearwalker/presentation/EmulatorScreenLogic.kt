...
internal fun buildActionHintForScreen(
    ...
    return when (state.screen) {
        DeviceScreen.Home -> {
            if (!hasWalkingCompanion) {
                "No companion active. ENTER: menu (Connect)"
        ...
        DeviceScreen.Companion -> "LEFT: previous (first exits), RIGHT: next, ENTER: return menu"
...
internal fun countCompanionEntriesFromEeprom(eeprom: ByteArray): Int {
    val walking = if (DeviceBinary.readWalkingCompanionSpecies(eeprom) != 0) 1 else 0
    val caught = DeviceBinary.countCaughtCompanion(eeprom)
    var items = 0
