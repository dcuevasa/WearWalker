...
internal fun resolveRadarScanEnter(
    ...
    val routeSlot = state.radarSignalRouteSlot.coerceIn(0, DeviceOffsets.COMPANION_SLOT_COUNT - 1)
    return RadarScanEnterResolution.UpdateState(
        state =
            state.copy(
                ...
                radarBattleCompanionSlot = routeSlot,
...
internal fun resolveRadarAttackTurn(
    ...
    if (enemyEvades) {
        ...
        persistMessage = "Attack missed. Wild encounter evaded and is counterattacking."
    ...
    if (playerHp <= 0) {
        ...
        applyLossPenalty = true
        shouldPersist = true
        persistMessage = "Your companion was overpowered."
    } else {
...
internal fun resolveRadarEvadeTurn(
    ...
    when {
        ...
        roll < counterChancePercent + stareChancePercent -> {
            ...
            persistMessage = "Both companions hesitated."
        }
            ...
            persistMessage = "Wild encounter ran away."
        }
    }
...
internal fun resolveRadarBattleMessageEnter(
    ...
    if (
        ...
        return RadarBattleMessageEnterResolution.ResetToHome(
            statusMessage = "Wild encounter got away. Returning Home.",
            outcome = false,
        )
    }
    ...
    if (state.radarBattleMessageOffset == offsets.wasTooStrong) {
        return RadarBattleMessageEnterResolution.ResetToHome(
            statusMessage = "Your companion was overpowered. Returning Home.",
            outcome = false,
        )
    }
    ...
    when (state.radarBattleMessageOffset) {
        offsets.foundSomething -> {
            return RadarBattleMessageEnterResolution.UpdateState(
                ...
                statusMessage = "A wild Companion appeared.",
            )
        }

        offsets.throwCaptureBall -> {
            return RadarBattleMessageEnterResolution.RefreshOnly("CaptureBall sequence in progress.")
        }
