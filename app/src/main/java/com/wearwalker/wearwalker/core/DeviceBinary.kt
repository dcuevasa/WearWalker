...
object DeviceBinary {
    ...
    fun countCaughtCompanion(eeprom: ByteArray): Int {
        var count = 0
        for (slot in 0 until DeviceOffsets.COMPANION_SLOT_COUNT) {
            if (readCaughtCompanionSpecies(eeprom, slot) != 0) {
                count += 1
            }
        }
        return count
    ...
    fun readWalkingCompanionSpecies(eeprom: ByteArray): Int {
        return readU16LE(eeprom, DeviceOffsets.ROUTE_INFO_OFFSET)
    }

    fun readCaughtCompanionSpecies(eeprom: ByteArray, slot: Int): Int {
        if (slot !in 0 until DeviceOffsets.COMPANION_SLOT_COUNT) return 0
        val offset = DeviceOffsets.CAUGHT_COMPANION_OFFSET + slot * DeviceOffsets.COMPANION_SUMMARY_SIZE
        return readU16LE(eeprom, offset)
    }

    fun readRouteCompanionSpecies(eeprom: ByteArray, slot: Int): Int {
        if (slot !in 0 until DeviceOffsets.COMPANION_SLOT_COUNT) return 0
        val offset = DeviceOffsets.ROUTE_COMPANION_LIST_OFFSET + slot * DeviceOffsets.COMPANION_SUMMARY_SIZE
        return readU16LE(eeprom, offset)
    }

    fun readRouteCompanionMinSteps(eeprom: ByteArray, slot: Int): Int {
        if (slot !in 0 until DeviceOffsets.COMPANION_SLOT_COUNT) return 0
        val offset = DeviceOffsets.ROUTE_INFO_OFFSET + 0x82 + slot * 2
        return readU16LE(eeprom, offset)
    }

    fun readRouteCompanionChance(eeprom: ByteArray, slot: Int): Int {
        if (slot !in 0 until DeviceOffsets.COMPANION_SLOT_COUNT) return 0
    ...
    fun findRouteSlotForSpecies(eeprom: ByteArray, species: Int): Int? {
        ...
        for (slot in 0 until DeviceOffsets.COMPANION_SLOT_COUNT) {
            if (readRouteCompanionSpecies(eeprom, slot) == species) {
                return slot
            }
        }
        return null
    ...
    fun recordCaughtCompanionFromRoute(eeprom: ByteArray, routeSlot: Int): Boolean {
        ...
        val targetOffset =
            DeviceOffsets.CAUGHT_COMPANION_OFFSET + targetSlot * DeviceOffsets.COMPANION_SUMMARY_SIZE
        System.arraycopy(eeprom, sourceOffset, eeprom, targetOffset, DeviceOffsets.COMPANION_SUMMARY_SIZE)
        return true
    }

    fun hasFreeCaughtCompanionSlot(eeprom: ByteArray): Boolean {
    ...
    fun replaceCaughtCompanionWithRoute(
        ...
        if (caughtSlot !in 0 until DeviceOffsets.COMPANION_SLOT_COUNT) {
            return false
        }
        if (routeSlot !in 0 until DeviceOffsets.COMPANION_SLOT_COUNT) {
            return false
        ...
        val targetOffset =
            DeviceOffsets.CAUGHT_COMPANION_OFFSET + caughtSlot * DeviceOffsets.COMPANION_SUMMARY_SIZE
        System.arraycopy(eeprom, sourceOffset, eeprom, targetOffset, DeviceOffsets.COMPANION_SUMMARY_SIZE)
        return true
    }
    ...
    fun readWalkingCompanionFriendship(eeprom: ByteArray): Int {
        return readUnsignedByte(eeprom, DeviceOffsets.ROUTE_FRIENDSHIP_OFFSET)
    }

    fun writeWalkingCompanionFriendship(
        eeprom: ByteArray,
        friendship: Int,
    ) {
