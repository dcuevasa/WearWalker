...
internal object RadarViewRenderer {
    fun renderScan(
        ...
        with(DeviceLcdRenderer) {
            ...
            val findText = decodeSprite(spriteData, TEXT_FIND_COMPANION, 96, 16)
    ...
    fun renderBattle(
        ...
        with(DeviceLcdRenderer) {
            ...
            val playerCompanion = decodeAnimatedSprite(spriteData, SMALL_COMPANION, 32, 24, animationFrame)
            val routeSlot = state.radarBattleCompanionSlot.coerceIn(0, 2)
            val enemyCompanion =
            ...
            if (!hideEnemy) {
                blit(frame, enemyCompanion, 32, 24, enemyX, enemyY)
            }
            blit(frame, playerCompanion, 32, 24, playerX, playerY)
            ...
            if (showBall) {
                val markerBall = decodeSprite(spriteData, COMPANIONBALL_8, 8, 8)
            ...
            LcdPreviewFrame(
                pixels = frame,
                hasVisualContent = hasNonBackground(enemyCompanion) || hasNonBackground(playerCompanion),
            )
        }
    ...
    fun renderSwap(
        ...
        with(DeviceLcdRenderer) {
            ...
            val selected = state.radarSwapCursor.coerceIn(0, DeviceOffsets.COMPANION_SLOT_COUNT - 1)
            ...
            val selectedRouteSlot =
                DeviceBinary.findRouteSlotForSpecies(
                    eeprom,
                    DeviceBinary.readCaughtCompanionSpecies(eeprom, selected),
                )
            ...
            val selectedName =
                if (selectedRouteSlot != null) {
                    decodeSprite(
                        spriteData,
                        ROUTE_COMPANION0_NAME + selectedRouteSlot.coerceIn(0, 2) * ROUTE_COMPANION_NAME_STRIDE,
                        80,
                        16,
                    )
                } else {
                    decodeSprite(spriteData, TEXT_NO_COMPANION_HELD, 96, 16)
                }
